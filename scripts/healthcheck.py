#!/usr/bin/env python3
"""Verify PostgreSQL reachability and required Phase 0 extensions."""
...
def main() -> None:
    try:
        ...
        print(f"Database health check failed: {error.__class__.__name__}", file=sys.stderr)
        raise SystemExit(1) from error

    vector_enabled = extension_row is not None and bool(extension_row[0])
    if not vector_enabled:
        print("Database health check failed: vector extension is unavailable", file=sys.stderr)
        raise SystemExit(1)
